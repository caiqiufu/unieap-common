 ...
 */
@Service
public class BizServiceBO {
	public Logger logger = Logger.getLogger(BizServiceBO.class);
	@Autowired
		...
		try {
			try {
				...
				if (!BizServiceUtils.authenticationCheck(requestInfo)) {
					return BizServiceUtils.exceptionEsblog(app_code, beginTime, requestInfo, requestInfoString, "20004",
							null);
				}
				if (!BizServiceUtils.requestLimitationCheck(requestInfo)) {
					return BizServiceUtils.exceptionEsblog(app_code, beginTime, requestInfo, requestInfoString, "20005",
							null);
				}
			} catch (Exception e) {
				return BizServiceUtils.exceptionEsblog(app_code, beginTime, requestInfo, requestInfoString,
			...
			if (bizHandler == null) {
				...
				return BizServiceUtils.exceptionEsblog(app_code, beginTime, requestInfo, requestInfoString, "10001",
	...
	public static BizHandler getProcessHandler(String code) throws Exception {
